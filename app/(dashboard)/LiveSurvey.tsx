...
const LiveGPSSurveyScreen = () => {
  ...
  const [activeLanesMap, setActiveLanesMap] =
  ...
  const [chainageData, setChainageData] = useState<ChainageData>({
    chainageStart: "",
    chainageEnd: "",
  });
...
const saveInspectionData=()=>{
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        const laneMap = res.data.data.reduce(
          (acc: ActiveLaneResponse, lane: any) => {
            if (lane.side === "L") {
    ...
    <SafeAreaView className="flex-1 bg-[#f4f8ff]">
      ...
      {showCamera && (
        <View className="h-96 mx-4 my-2 rounded-xl overflow-hidden relative">
          ...
          <TouchableOpacity
            className="absolute top-2 right-2 bg-black/50 px-3 py-1 rounded"
      ...
      <ScrollView className="flex-1 bg-[#f4f8ff]">
        ...
        {/* Map Section */}
        <View className="mx-4 rounded-2xl overflow-hidden h-100 mb-2">
          <MapView
            style={{ flex: 1, height: 275, overflow: "hidden" }}
            showsCompass
            region={location}
            showsUserLocation
